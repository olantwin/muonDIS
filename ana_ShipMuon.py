# analyze muon background
# /media/Data/HNL/PythiaGeant4Production/pythia8_Geant4_total.root
import os
import ROOT
...
def origin(sTree, it):
    ...
    if im == 0:
        ...
        rz_inter = ROOT.TMath.Sqrt(at.GetStartX()**2 + at.GetStartY() **
                                   2), at.GetStartZ()
...
fchain = []
fchainRec = []
# first time trying to read a chain of Fairship files, doesn't seem to
# work out of the box, try some tricks.
testdir = '.'
if path != '':
    testdir = path
if withChain > 0:
    for prefix in prefixes:
        for i in range(1, 10):
            if not prefix + str(i) in os.listdir(testdir):
                continue
...
def bookHist(detName):
    for mu in ['', '_mu', '_muV0']:
        ...
        if detName.find('LS') < 0:
            ...
            ut.bookHist(h, tag, 'z phi ' + detName, 100, -50., 50., 100, -1.,
                        1.)
        ut.bookHist(h, tag + '_E', 'deposited energy MeV ' + detName, 100, 0.,
                    2.)
        ut.bookHist(h, tag + '_P', 'particle mom GeV ' + detName, 500, 0., 50.)
        ut.bookHist(h, tag + '_LP', 'particle mom GeV ' + detName, 100, 0., 1.)
        ut.bookHist(h, tag + '_OP', 'original particle mom GeV ' + detName,
                    400, 0., 400.)
        ut.bookHist(h, tag + '_id', 'particle id ' + detName, 5001, -2499.5,
                    2499.5)
        ut.bookHist(h, tag + '_mul', 'multiplicity of hits/tracks ' + detName,
                    100, -0.5, 99.5)
        ut.bookHist(h, tag + '_evmul', 'multiplicity of hits/event ' + detName,
                    100, -0.5, 9999.5)
        ut.bookHist(h, tag + '_origin', 'r vs z', 100, ztarget, totl, 100, 0.,
                    12.)
        ut.bookHist(h, tag + '_originmu', 'r vs z', 100, ztarget, totl, 100,
                    0., 12.)
...
ut.bookHist(h, 'prop_deltaElec', 'energy of delta elec / muon energy', 100, 0.,
            1.)
...
def BigEventLoop():
    ...
    for fn in fchain:
        ...
        if parallel:
            ...
            processes.append(
                mp.Process(target=executeOneFile, args=(fn, output, pid)))
            pid += 1
        else:
            processes.append(fn)
# Run processes
    ...
    for mu in ['', '_mu', '_muV0']:
        for x in [
            ...
            for j in hMuon:
                ...
                if first:
                    h[newh] = h[tag].Clone(newh)
                    h[newh].SetTitle(h[tag].GetTitle().split(' ')[0] + ' ' +
                                     newh)
                    first = False
                else:
                    rc = h[newh].Add(h[tag])
...
def executeOneFile(fn, output=None, pid=None):
    ...
    sTree.GetEntry(0)
    hitContainers = [
        sTree.vetoPoint, sTree.muonPoint, sTree.EcalPointLite,
        sTree.strawtubesPoint, sTree.ShipRpcPoint, sTree.TargetPoint
    ]
    ntot = 0
    for n in range(nEvents):
        ...
        rc = h['prop_deltaElec'].Fill(
            esum / sTree.MCTrack[0].GetP(),
            w)  # until energy is really made persistent GetEnergy()
        for c in hitContainers:
            for ahit in c:
                ...
                if ahit.GetName() == 'strawtubesPoint':
                    ...
                    # not needed for lite collection: if
                    # abs(ahit.GetPdgCode())==12 or abs(ahit.GetPdgCode())==14
                    # : continue
                    detName = 'Ecal'
                    ...
                    if detID not in logVols:
                        ...
                        if not detName in histlistAll.values():
                ...
                if not trackID < 0:
                    ...
                    aTrack.GetMomentum(
                        mom
                    )  # ! this is not momentum of particle at Calorimeter place
                    phit = mom.Mag() / u.GeV
                if abs(pdgID) == 13:
                    mu = '_mu'
                ...
                detName = detName + mu
                if detName.find('LS') < 0:
                    rc = h[detName].Fill(x / u.m, y / u.m, w)
                else:
                    rc = h[detName].Fill(ahit.GetZ() / u.m,
                                         ROOT.TMath.ATan2(y, x) /
                                         ROOT.TMath.Pi(), w)
                ...
                if not trackID < 0:
                    r = ROOT.TMath.Sqrt(aTrack.GetStartX()**2 +
                                        aTrack.GetStartY()**2) / u.m
                    rc = h['origin'].Fill(aTrack.GetStartZ() / u.m, r, w)
                    rc = h[detName + '_origin'].Fill(aTrack.GetStartZ() / u.m,
                                                     r, w)
                    if abs(pdgID) == 13:
                        rc = h[detName + '_originmu'].Fill(aTrack.GetStartZ() /
                                                           u.m, r, w)
                    ...
                    if trackID > 0:
                        ...
                        rc = h['porigin'].Fill(
                            ...
                            ROOT.TMath.Sqrt(aTrack.GetStartX()**2 +
                                            aTrack.GetStartY()**2) / u.m, w)
                    rc = h['mu-inter'].Fill(rz_inter[1] / u.m, rz_inter[0] /
                                            u.m, w)
...
def makePlots(nstations):
    ...
    ut.bookCanvas(
        ...
        cy=cxcy[nstations][1])
    ut.bookCanvas(
        h, key='ResultsV', title='origin info', nx=600, ny=400, cx=1, cy=1)
                ...
                for ahit in sTree.strawtubesPoint:
                    if ahit.GetTrackID() == mcPartKey:
                        fout.write('P when making hit %i, %8.2F \n' % (
                            ahit.PdgCode(),
                            ROOT.TMath.Sqrt(ahit.GetPx()**2 + ahit.GetPy()**2 +
                                            ahit.GetPz()**2) / u.GeV))
                        break
                if not mcPart:
                    continue
...
def muDISntuple(fn):
    ...
    h['ntuple'] = ROOT.TNtuple('muons', 'muon flux VetoCounter',
                               'id:px:py:pz:x:y:z:w')
    f = ROOT.TFile(fn)
    sTree = f.cbmsim
    nEvents = sTree.GetEntries()
...
def analyzeConcrete():
    ...
    h['ntuple'] = ROOT.TNtuple('muons', 'muon flux concrete',
                               'id:px:py:pz:x:y:z:w')
    for m in ['', 'mu', 'V0']:
    ...
    for fn in fchainRec:
        ...
        for n in range(nEvents):
            ...
            for ahit in sTree.vetoPoint:
                ...
                if sTree.MCTrack.GetEntries() > 1:
                    wg = sTree.MCTrack[
                        1].GetWeight()  # also works for neutrinos
                else:
                    wg = sTree.MCTrack[
                        0].GetWeight()  # also works for neutrinos
...
def extractMuCloseByEvents(single=None):
    ...
    for fn in fchainRec:
        ...
        for n in range(nEvents):
            ...
            n = 0
            for c in [
                    sTree.vetoPoint, sTree.strawtubesPoint, sTree.ShipRpcPoint
            ]:
                for ahit in c:
...
def reDraw(fn):
    ...
    if 'tc' not in h:
        h['tc'] = ROOT.TFile(fn)
    for x in [
            'ResultsI', 'ResultsII', 'ResultsImu', 'ResultsImuV0',
            'ResultsIII', 'ResultsIV', 'ResultsV'
    ]:
...
def debugGeoTracks():
    for i in range(sTree.GetEntries()):
        ...
        for gt in sTree.GeoTracks:
            print i, n, gt.GetFirstPoint()[2], gt.GetLastPoint()[
                2], gt.GetParticle().GetPdgCode(), gt.GetParticle().P()
            n += 1
...
def originOfMuon(fout, n, fn, nEvents):
    ...
    N = n3 * ni + n
    ntupl.GetEvent(N)
    P = ROOT.TMath.Sqrt(ntupl.pz * ntupl.pz + ntupl.py * ntupl.py + ntupl.px *
                        ntupl.px)
...
def pers():
    ...
    for x in h:
        if isinstance(h[x], type(ROOT.TCanvas())):
            h[x].Update()
            tn = h[x].GetName() + '.png'
            h[x].Print(tn)
            ...
            for i in range(len(lp)):
                if not lp[i] in os.listdir(prefix):
...
def makeNicePrintout(x=['rareEvents_61-62.txt', 'rareEvents_71-72.txt']):
    ...
    # print a table
    print '%4s %8s %8s %4s %8s %8s %8s %8s %8s  %8s ' % (
        'nr', 'origin', 'pythiaID', 'ID', 'p_orig', 'p_hit', 'chi2', 'weight',
        'file', 'cor w')
    ...
    for i in range(len(tmp)):
        ...
        if float(tr['p_hit']) > 3:
            muonrate3 += corw
        print '%4i %8s %8s %4s %8s %8s %8s %8s %8s  %8s ' % (
            i, tr['origin'], tr['pytiaid'], tr['id_hit'], tr['o-mom'],
            tr['p_hit'], tr['chi2'], tr['w'], tr['file'], corw)
