...
nEvents = 100
firstEvent = 0
# True = all processes if "c" only ccbar -> HNL, if "b" only bbar -> HNL,
# and for darkphotons: if meson = production through meson decays, TBD:
# proton brem, QCD prod.
inclusive = 'c'
deepCopy = False  # False = copy only stable particles to stack, except for HNL events
# option to be set with -A to enable only charm decays, charm x-sec measurement
charmonly = False
HNL = True
DarkPhoton = False
RPVSUSY = False
...
try:
    opts, args = getopt.getopt(sys.argv[1:], 'D:FHPu:n:i:f:g:c:hqv:s:l:A:Y:i:m:co:', [
...
if simEngine == 'muonDIS':
    ...
    mu_start, mu_end = ship_geo.Chamber1.z - \
        ship_geo.chambers.Tub1length - 10. * u.cm, ship_geo.TrackStation1.z
...
# manipulate G4 geometry to enable magnetic field in active shielding, VMC
# can't do it.
...
saveBasicParameters.execute('%s/geofile_full.%s.root' %
                            (outputDir, tag), ship_geo)
